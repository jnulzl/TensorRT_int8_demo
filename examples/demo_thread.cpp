...
void yolov6_thread_func(const std::string& input_name, const std::string& output_name,
                        ...
                        const std::string& input_src, int is_save_res)
{
    ...
    while (true)
    {
        ...
        std::cout << "Thread id = " << std::this_thread::get_id() << " Frame = " << frame_id << " Batch = " << config_tmp.batch_size << " TensorRT process time = " << std::chrono::duration_cast<std::chrono::microseconds>(finishTP1 - startTP).count() << " us" << std::endl;
        for (int bs = 0; bs < config_tmp.batch_size; ++bs)
        {
...
//
//    }
    std::thread t1(yolov6_thread_func, input_name, output_name, weights_path, net_inp_width, net_inp_height,
                       det_num_cls, batch_size, device_id, input_src, is_save_res);

//    std::thread t2(yolov6_thread_func, input_name, output_name, weights_path, net_inp_width, net_inp_height,
//                   det_num_cls, batch_size, device_id, input_src, is_save_res);
